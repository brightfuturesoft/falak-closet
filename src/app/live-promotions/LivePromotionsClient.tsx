...
function CountdownTimer({ saleEndsAt }: CountdownTimerProps) {
  ...
  if (!mounted) {
    return (
      <div className="flex items-center gap-1.5 font-mono text-[#A80C14] font-extrabold text-sm bg-[#FDF2F3] px-3 py-1.5 rounded-full border border-[#F8D2D5] shadow-xs">
  ...
  return (
    <div className="flex items-center gap-1.5 font-mono text-[#A80C14] font-extrabold text-sm bg-[#FDF2F3] px-3 py-1.5 rounded-full border border-[#F8D2D5] shadow-xs animate-pulse">
      <Clock className="w-4 h-4" />
      <span>
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-10 space-y-10 pb-28 lg:pb-12 text-stone-900">
      <div className="text-center max-w-xl mx-auto space-y-3">
        <span className="px-3.5 py-1 bg-[#FDF2F3] text-[#A80C14] border border-[#F8D2D5] text-xs font-bold rounded-full uppercase tracking-wider inline-flex items-center gap-1.5">
          <Flame className="w-4 h-4 fill-[#A80C14]" /> Special Discounts & Vouchers
        </span>
      ...
      {banners.length === 0 ? (
        <div className="py-16 text-center bg-white rounded-3xl border border-[#F8D2D5] shadow-xs max-w-xl mx-auto space-y-3">
          <Flame className="w-10 h-10 mx-auto text-[#A80C14] fill-[#A80C14]/20 animate-pulse" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {banners.map((promo) => (
            <div
              key={promo.id}
              className="p-6 bg-white rounded-3xl border border-[#F8D2D5] shadow-xs space-y-4 flex flex-col justify-between hover:border-[#A80C14]/30 transition-all duration-300"
            >
              <div className="space-y-3">
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="px-2.5 py-1 bg-[#FDF2F3] text-[#A80C14] border border-[#F8D2D5] text-[10px] font-extrabold rounded-lg uppercase">
                      {promo.discountBadge}
                    </span>
                    {promo.minSpend !== null && (
              ...
              {promo.code && (
                <div className="pt-4 border-t border-[#F8D2D5] flex items-center justify-between gap-2">
                  ...
                  <button
                    ...
                    className="px-4 py-2 bg-[#A80C14] hover:bg-[#8C0A10] text-white text-xs font-bold rounded-2xl transition-colors shadow-xs flex items-center gap-1.5 cursor-pointer"
                  >
      ...
      {flashProducts.length > 0 && (
        <div className="space-y-6 pt-6 border-t border-[#F8D2D5]">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              <h2 className="font-sans text-2xl font-extrabold text-stone-900 flex items-center gap-2">
                <Flame className="w-6 h-6 text-[#A80C14] fill-[#A80C14]" /> Limited-Time Flash Sale Items
              </h2>
