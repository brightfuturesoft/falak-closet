...
export function ValuePropsSection({ items }: { items: ValuePropItem[] }) {
  return (
    <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full" aria-label="Why choose Falak Closet">
      <div className="bg-white border border-[#F8D2D5] rounded-3xl p-4 sm:p-10 space-y-5 sm:space-y-8 shadow-xs">
        <div className="text-center max-w-2xl mx-auto space-y-1.5 sm:space-y-2">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-[#FDF2F3] border border-[#F8D2D5] rounded-full text-[10px] sm:text-xs text-[#A80C14] font-bold uppercase tracking-wider">
            <Sparkles className="w-3.5 h-3.5" />
            <span>The Falak Closet Standard</span>
          </div>
          <h2 className="font-serif text-xl sm:text-3xl font-extrabold text-[#0D153A] leading-tight">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6">
          {items.map((item, idx) => {
            ...
            return (
              <div
                key={`${item.title}-${idx}`}
                className="bg-white p-3.5 sm:p-5 rounded-2xl border border-stone-200/70 space-y-1.5 sm:space-y-3 shadow-xs hover:border-[#F8D2D5] hover:shadow-md transition-all"
              >
                <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-[#FDF2F3] text-[#A80C14] flex items-center justify-center">
                  <Icon className="w-[18px] h-[18px] sm:w-5 sm:h-5" />
                </div>
                <h3 className="font-serif font-bold text-xs sm:text-sm text-[#0D153A] leading-snug">
                  {item.title}
                </h3>
