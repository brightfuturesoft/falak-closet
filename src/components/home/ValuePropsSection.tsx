...
export function ValuePropsSection({ items }: { items: ValuePropItem[] }) {
  return (
    <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full" aria-label="Why choose Falak Closet">
      <div className="bg-[#FFFBF0] border border-[#F2C76E]/60 rounded-3xl p-4 sm:p-10 space-y-5 sm:space-y-8 shadow-xs">
        <div className="text-center max-w-2xl mx-auto space-y-1.5 sm:space-y-2">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-[#F2C76E]/20 border border-[#F2C76E]/60 rounded-full text-[10px] sm:text-xs text-[#9B050B] font-bold uppercase tracking-wider">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6">
          {items.map((item, idx) => {
            ...
            return (
              <div
                key={`${item.title}-${idx}`}
                className="bg-white p-3.5 sm:p-5 rounded-2xl border border-[#F2C76E]/40 space-y-1.5 sm:space-y-3 shadow-xs hover:border-[#9B050B]/30 hover:shadow-md transition-all"
              >
                <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-[#9B050B]/10 text-[#9B050B] flex items-center justify-center">
