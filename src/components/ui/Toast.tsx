        ...
        {toasts.map((toast) => (
          ...
          >
            <div className="flex items-center gap-3 min-w-0">
              {toast.image ? (
                <div className="relative w-12 h-14 rounded-xl overflow-hidden bg-stone-100 flex-shrink-0 border border-[#F8D2D5]">
                  <Image src={toast.image} alt={toast.title} fill sizes="48px" className="object-cover" />
                </div>
              ) : (
                <div className="w-10 h-10 rounded-full bg-[#FDF2F3] text-[#A80C14] flex items-center justify-center flex-shrink-0 font-bold border border-[#F8D2D5]/45">
                  {toast.type === 'cart' ? <ShoppingBag className="w-5 h-5" /> : <Heart className="w-5 h-5 text-[#A80C14] fill-[#A80C14]" />}
                </div>
              )}

              <div className="min-w-0">
                <div className="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-wider text-[#A80C14]">
            ...
            <div className="flex items-center gap-2">
              {toast.actionLink && (
                <Link
                  ...
                  className="px-3 py-1.5 bg-[#FFF5F7] border border-[#F8D2D5] hover:bg-[#A80C14] text-[#A80C14] hover:text-white text-[11px] font-bold rounded-xl transition-all duration-300 flex items-center gap-1 whitespace-nowrap shadow-xs cursor-pointer active:scale-95"
                >
